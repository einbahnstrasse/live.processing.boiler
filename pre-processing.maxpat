...
, 			{
				"box" : 				{
					...
					"text" : "When we process a signal using real-time audio effects and other synthesis routines, it is not usually the case that the \"raw\" microphone signal needs to be processed... \n\nPROTIP: We only need to use the \"raw\" mic or live signal from the stage when we faithfully AMPLIFY that signal. On the other hand, when we SYNTHESIZE something based on that signal, it first needs to be \"cleaned\" (Fr., \"nettoyée,\" Gr. \"gereinigt\") to eliminate parts of the signal that do not serve the processing algorithms we use.\n\nWe don't need extremely low noise for our processing, so we reject these parts of our input signal using a filter.",
